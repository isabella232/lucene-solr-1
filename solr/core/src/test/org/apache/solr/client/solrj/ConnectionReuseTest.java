...
public class ConnectionReuseTest extends AbstractFullDistribZkTestBase {
  ...
  public void doTest() throws Exception {
    URL url = new URL(((HttpSolrServer) clients.get(0)).getBaseURL());
    
    SolrServer client;
    HttpClient httpClient = HttpClientUtil.createClient(null);
    int rndClient = 0;//random().nextInt(3);
    if (rndClient == 0) {
      client = new ConcurrentUpdateSolrServer(url.toString(), httpClient, 6, 1); // currently only testing with 1 thread
    } else if (rndClient == 1)  {
      client = new HttpSolrServer(url.toString(), httpClient);
    } else if (rndClient == 2) {
      client = new CloudSolrServer(zkServer.getZkAddress(), random().nextBoolean(), httpClient);
      ((CloudSolrServer) client).setParallelUpdates(random().nextBoolean());
      ((CloudSolrServer) client).setDefaultCollection(DEFAULT_COLLECTION);
      ((CloudSolrServer) client).getLbServer().setConnectionTimeout(30000);
      ((CloudSolrServer) client).getLbServer().setSoTimeout(60000);
    } else {
    ...
    for (int j = 0; j < cnt1; j++) {
      for (int i = 0; i < cnt2; i++) {
        ...
        if (!done && i > 0 && i < cnt2 - 1 && client instanceof ConcurrentUpdateSolrServer && random().nextInt(10) > 8) {
          queueBreaks++;
          done = true;
          Thread.sleep(350); // wait past streaming client poll time of 250ms
        }
      }
      if (client instanceof ConcurrentUpdateSolrServer) {
        ((ConcurrentUpdateSolrServer) client).blockUntilFinished();
      }
    }
