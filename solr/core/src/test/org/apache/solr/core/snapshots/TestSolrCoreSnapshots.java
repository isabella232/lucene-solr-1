...
public class TestSolrCoreSnapshots extends SolrCloudTestCase {
  ...
  public void testBackupRestore() throws Exception {
    ...
    SolrServer adminClient = new HttpSolrServer(cluster.getJettySolrRunners().get(0).getBaseUrl().toString());
    SolrServer masterClient = new HttpSolrServer(replica.getCoreUrl());
    try {
  ...
  public void testIndexOptimization() throws Exception {
    ...
    try {
      ...
      // Before invoking optimize command, verify that the index directory contains multiple commits (including the one we snapshotted earlier).
      {
        ...
        assertTrue(getCommitWithGen(commits, metaData.getGenerationNumber()).isPresent());
      }
      ...
      Collection<IndexCommit> commits = listCommits(metaData.getIndexDirPath());
      assertTrue(commits.size() == 1);
      assertFalse(getCommitWithGen(commits, metaData.getGenerationNumber()).isPresent());
    } finally {
      if (adminClient != null) {
        adminClient.shutdown();
      }
  ...
  private SnapshotMetaData createSnapshot (SolrServer adminClient, String coreName, String commitName) throws Exception {
    ...
    for (SnapshotMetaData m : snapshots) {
      if (commitName.equals(m.getName())) {
  ...
  private void deleteSnapshot(SolrServer adminClient, String coreName, String commitName) throws Exception {
  ...
  private Collection<SnapshotMetaData> listSnapshots(SolrServer adminClient, String coreName) throws Exception {
  ...
  private List<IndexCommit> listCommits(String directory) throws Exception {
    SimpleFSDirectory dir = new SimpleFSDirectory(Paths.get(directory).toFile());
    try {
  ...
  private Optional<IndexCommit> getCommitWithGen (Collection<IndexCommit> commits, long gen) {
