...
public class BlockCacheTest extends LuceneTestCase {
  ...
  public void testCacheConcurrent() throws Exception {
    ...
    final EvictionListener<Long,Val> listener = new EvictionListener<Long,Val>() {
      @Override
      public void onEviction(Long k, Val v) {
        assert v.key == k;
        if (!v.live.compareAndSet(true, false)) {
          throw new RuntimeException("listener called more than once! k=" + k + " v=" + v);
          // return;  // use this variant if listeners may be called more than once
        }
        removals.incrementAndGet();
      }
    };
    final ConcurrentLinkedHashMap<Long,Val> cache = new ConcurrentLinkedHashMap.Builder<Long,Val>()
            .maximumWeightedCapacity(maxEntries).concurrencyLevel(maxEntries).initialCapacity(maxEntries).listener(listener).build();
      ...
      threads[i] = new Thread() {
        ...
        public void test() {
          ...
          if (v == null || updateAnyway && r.nextBoolean()) {
            v = new Val();
            v.key = k;
            Val prev = cache.put(k, v);
            if (prev != null) listener.onEviction(k, prev);  // unlike Caffeine, we need to call the removal listener ourselves here.
    ...
    Thread.sleep(1000); // wait for removal listeners to be called?
