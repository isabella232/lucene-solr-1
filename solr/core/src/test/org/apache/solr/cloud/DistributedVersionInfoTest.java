...
public class DistributedVersionInfoTest extends AbstractFullDistribZkTestBase {
  ...
  protected void testReplicaVersionHandling() throws Exception {
    ...
    SolrServer client = new HttpSolrServer(replica.getCoreUrl());
    try {
      ...
      throws Exception
  {
    ...
    HttpSolrServer leaderSolr = getHttpSolrServer(leader);
    List<HttpSolrServer> replicas = new ArrayList<HttpSolrServer>(notLeaders.size());
    for (Replica r : notLeaders)
      replicas.add(getHttpSolrServer(r));
  ...
  protected HttpSolrServer getHttpSolrServer(Replica replica) throws Exception {
    return new HttpSolrServer(replica.getCoreUrl());
  }
  ...
  protected Long assertDocExists(HttpSolrServer solr, String coll, String docId, Long expVers) throws Exception {
  ...
  protected boolean reloadCollection(Replica replica, String testCollectionName) throws Exception {
    ...
    HttpSolrServer client = new HttpSolrServer(coreProps.getBaseUrl());
    try {
