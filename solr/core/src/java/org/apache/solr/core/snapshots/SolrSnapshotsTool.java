...
public class SolrSnapshotsTool implements Closeable {
  ...
  private static final String EXPORT_SNAPSHOT = "export-snapshot";
  private static final String HELP = "help";
  private static final String COLLECTION = "c";
  private static final String TEMP_DIR = "t";
  private static final String DEST_ON_HDFS = "d";
  ...
  public SolrSnapshotsTool(String solrZkEnsemble) {
    solrClient = new CloudSolrServer(solrZkEnsemble);
  }
    ...
    try {
      ...
      for (CollectionSnapshotMetaData m : snaps) {
        if (snapshotName.equals(m.getName())) {
          ...
          System.out.println("Time of creation: " + dateFormat.format(new Date(m.getCreationTime())));
  ...
  public static void main(String[] args) throws IOException {
    ...
    options.addOption(null, HELP, true, "This command will print the help message for the snapshots related commands.");
    options.addOption(TEMP_DIR, true, "This parameter specifies the path of a temporary directory on local filesystem"
        + " during snapshot export operation.");
    options.addOption(DEST_ON_HDFS, true, "This parameter specifies the path on HDFS where the snapshot related"
    ...
    options.addOption(HDFS_PATH_PREFIX, true, "This parameter specifies the HDFS URI prefix to be used during snapshot export preparation");
    ...
    if (cmd.hasOption(CREATE) || cmd.hasOption(DELETE) || cmd.hasOption(LIST) || cmd.hasOption(DESCRIBE)
      ...
      try (SolrSnapshotsTool tool = new SolrSnapshotsTool(cmd.getOptionValue(SOLR_ZK_ENSEMBLE))) {
        if (cmd.hasOption(CREATE)) {
          ...
          String hdfsOpDir = requiredArg(options, cmd, DEST_ON_HDFS);
          Optional<String> pathPrefix = Optional.fromNullable(cmd.getOptionValue(HDFS_PATH_PREFIX));
  ...
  private static void printHelp(Options options) {
    ...
    formatter.printHelp("SolrSnapshotsTool", options, true);
  }
