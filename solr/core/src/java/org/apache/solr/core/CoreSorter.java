...
public class CoreSorter {
  ...
  public CoreSorter init(CoreContainer cc) {
    ...
    for (CloudDescriptor cloudDescriptor : getCloudDescriptors()) {
      ...
      CountsForEachShard c = new CountsForEachShard(0, 0);
  ...
  static class CountsForEachShard {
    public int totalReplicasInDownNodes = 0, totalReplicasInLiveNodes = 0;

    public CountsForEachShard(int totalReplicasInDownNodes,  int totalReplicasInLiveNodes) {
    ...
    public String toString() {
      return "down : " + totalReplicasInDownNodes + " , up :  " + totalReplicasInLiveNodes;
    }
  ...
  Collection<CloudDescriptor> getCloudDescriptors() {
    Collection<CloudDescriptor> result = new ArrayList<>(cc.getCores().size());
    for (SolrCore core : cc.getCores()) {
      result.add(core.getCoreDescriptor().getCloudDescriptor());
    }
    return result;
  }
}
