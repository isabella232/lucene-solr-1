...
public class HdfsDirectoryFactory extends CachingDirectoryFactory {
  ...
  protected Directory create(String path, DirContext dirContext) throws IOException {
    ...
    boolean blockCacheEnabled = params.getBool(
        BLOCKCACHE_ENABLED, true);
    boolean blockCacheReadEnabled = params.getBool(
        BLOCKCACHE_READ_ENABLED, true);
    boolean blockCacheWriteEnabled = params.getBool(
        BLOCKCACHE_WRITE_ENABLED, true);
    Directory dir = null;
    ...
    if (blockCacheEnabled && dirContext != DirContext.META_DATA) {
      ...
      BufferStore.init(params, metrics);
    ...
    boolean nrtCachingDirectory = params.getBool(
        NRTCACHINGDIRECTORY_ENABLE, true);
    if (nrtCachingDirectory) {
      double nrtCacheMaxMergeSizeMB = params.getInt(
          NRTCACHINGDIRECTORY_MAXMERGESIZEMB, 16);
      double nrtCacheMaxCacheMB = params.getInt(
          NRTCACHINGDIRECTORY_MAXCACHEMB, 192);
  ...
  public String getDataHome(CoreDescriptor cd) throws IOException {
    ...
    if (cd.getCloudDescriptor() != null) {
      path = URLEncoder.encode(cd.getCloudDescriptor().getCollectionName(), "UTF-8") + "/"
