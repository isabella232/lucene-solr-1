...
public class LocalFileSystemRepository implements BackupRepository {
  ...
  public IndexInput openInput(URI dirPath, String fileName, IOContext ctx) throws IOException {
    try (FSDirectory dir = new SimpleFSDirectory(Paths.get(dirPath).toFile(), NoLockFactory.getNoLockFactory())) {
  ...
  public String[] listAll(URI dirPath) throws IOException {
    ...
    if(!dir.isDirectory()) {
      throw new IllegalArgumentException("The specified path " + dirPath + " is not a directory.");
    }
    return dir.list();
  }
  ...
  public void copyFileFrom(Directory sourceDir, String fileName, URI dest) throws IOException {
    try (FSDirectory dir = new SimpleFSDirectory(Paths.get(dest).toFile(), NoLockFactory.getNoLockFactory())) {
      sourceDir.copy(dir, fileName, fileName, DirectoryFactory.IOCONTEXT_NO_CACHE);
    }
  }

  @Override
  public void copyFileTo(URI sourceDir, String fileName, Directory dest) throws IOException {
    try (FSDirectory dir = new SimpleFSDirectory(Paths.get(sourceDir).toFile(), NoLockFactory.getNoLockFactory())) {
      dir.copy(dest, fileName, fileName, DirectoryFactory.IOCONTEXT_NO_CACHE);
    }
  }
