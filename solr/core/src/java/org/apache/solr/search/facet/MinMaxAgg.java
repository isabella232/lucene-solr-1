...
public class MinMaxAgg extends SimpleAggValueSource {
  ...
  public SlotAcc createSlotAcc(FacetContext fcontext, int numDocs, int numSlots) throws IOException {
    ...
    if (sf.getType() instanceof TrieField) {
      switch ( ((TrieField)sf.getType()).getType() ) {
        case FLOAT:
        case DOUBLE:
    ...
    } else if (sf.getType() instanceof DateField) {
  ...
  class SingleValuedOrdAcc extends OrdAcc {
    ...
    public void setNextReader(AtomicReaderContext readerContext) throws IOException {
      ...
      if (subDvs != null) {
        ...
        // BP assert readerContext.ord==0 || topLevel.getValueCount() == 0;
        subDv = FieldUtil.getSortedDocValuesSegment(field, topLevel, readerContext);
      }
    }
