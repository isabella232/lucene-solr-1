...
public abstract class FacetFieldProcessor extends FacetProcessor<FacetField> {
  ...
  public int collectFirstPhase(DocSet docs, int slot) throws IOException {
    int num = -1;
    if (collectAcc != null) {
  ...
  public void collectFirstPhase(int segDoc, int slot) throws IOException {
    if (collectAcc != null) {
  ...
  SimpleOrderedMap<Object> findTopSlots(final int numSlots, final int slotCardinality,
    ...
    if (indexOrderAcc != null && indexOrderAcc != sortAcc) {
      orderPredicate = new BiPredicate<Slot, Slot>() {
        @Override
  ...
  public void setNextReaderFirstPhase(AtomicReaderContext ctx) throws IOException {
    if (collectAcc != null) {
  ...
  private SimpleOrderedMap<Object> refineBucket(Object bucketVal, boolean skip, Map<String,Object> facetInfo) throws IOException {
    ...
    // JF backport of toNativeType specific to int/float
    if (org.apache.lucene.document.FieldType.NumericType.INT == ft.getNumericType()) {
      bucketVal = ((Number)bucketVal).intValue();
    } else if (org.apache.lucene.document.FieldType.NumericType.FLOAT == ft.getNumericType()) {
      bucketVal = ((Number)bucketVal).floatValue();
    }
