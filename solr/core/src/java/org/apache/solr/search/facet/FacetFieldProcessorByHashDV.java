...
class FacetFieldProcessorByHashDV extends FacetFieldProcessor {
  ...
  FacetFieldProcessorByHashDV(FacetContext fcontext, FacetField freq, SchemaField sf) {
    ...
    /** this will cause fieldinfo merges each time just for an error check.
    FieldInfo fieldInfo = fcontext.searcher.getSlowAtomicReaderReader().getFieldInfos().fieldInfo(sf.getName());
    if (fieldInfo != null &&
  ...
  private SimpleOrderedMap<Object> calcFacets() throws IOException {
    ...
    return super.findTopSlots(table.numSlots(), table.cardinality(),
            new IntFunction<Comparable>() {
              @Override
              public Comparable apply(int slotNum) {
                return calc.bitsToValue(table.vals[slotNum]);
              }
            }, // getBucketValFromSlotNum
            new Function<Comparable, String>() {
              @Override
              public String apply(Comparable val) {
                return calc.formatValue(val);
              }
            }); // getFieldQueryVal
  }
  ...
  private void collectDocs() throws IOException {
    if (calc instanceof TermOrdCalc) { // Strings
      ...
      final SortedDocValues globalDocValues = FieldUtil.getSortedDocValues(fcontext.qcontext, sf, null);
      ((TermOrdCalc)calc).lookupOrdFunction = new IntFunction<BytesRef>() {
        @Override
        public BytesRef apply(int ord) {
          return globalDocValues.lookupOrd(ord);
        }
      };

      DocSetUtil.collectSortedDocSet(fcontext.base, fcontext.searcher.getIndexReader(), new Collector() {
        ...
        public void setNextReader(AtomicReaderContext ctx) throws IOException {
          ...
          if (globalDocValues instanceof MultiDocValues.MultiSortedDocValues) {
            ...
            // FieldCache docValues, we need to create a segment view
            docValues = FieldUtil.getSortedDocValuesSegment(fcontext.qcontext, sf, ctx);
          }
        }
      ...
      // We don't need to support SortedNumericDocValues in 4.10, Solr didn't use them (used string based SortedSet)
      ...
      DocSetUtil.collectSortedDocSet(fcontext.base, fcontext.searcher.getIndexReader(), new Collector() {
        ...
        public void setNextReader(AtomicReaderContext ctx) throws IOException {
          setNextReaderFirstPhase(ctx);
          values = FieldUtil.getNumericDocValues(sf, ctx);
          docsWithField = FieldUtil.getDocsWithField(sf, ctx);
        }
