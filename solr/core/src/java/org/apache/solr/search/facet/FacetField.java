...
public class FacetField extends FacetRequestSorted {
  ...
  public FacetProcessor createFacetProcessor(FacetContext fcontext) {
    ...
    if (fcontext.facetInfo != null) {
      ...
      if (!multiToken) {
        // all single-valued should be able to be refined by ArrayDV method
        return new FacetFieldProcessorByArrayDV(fcontext, this, sf);
      } else {
        // If multi-valued, we want to avoid using SortedSetDocValues unless we have real docValues field
        if (sf.hasDocValues()) {
          return new FacetFieldProcessorByArrayDV(fcontext, this, sf);
        } else {
    ...
    if (ntype != null) {
      ...
      if (mincount == 0 && !multiToken) {  // For BP, we added !multiToken since UIF can support zeros
        throw new SolrException(SolrException.ErrorCode.BAD_REQUEST,
    ...
    if (sf.hasDocValues()) { // BP, only use docvalues (SortedSetDocValues) if we have *real* docvalues
