...
class FacetFieldProcessorByEnumTermsStream extends FacetFieldProcessor implements Closeable {
  ...
  private void setup() throws IOException {
    ...
    Terms terms = fcontext.searcher.getAtomicReader().terms(sf.getName());
    ...
    try {
      while (term != null) {
        ...
        if (hasSubFacets || df >= minDfFilterCache) {
          ...
          postingsEnum = termsEnum.docs(null, postingsEnum, DocsEnum.FLAG_NONE);


          if (postingsEnum instanceof MultiDocsEnum) {
            MultiDocsEnum.EnumWithSlice[] subs = ((MultiDocsEnum) postingsEnum).getSubs();
            int numSubs = ((MultiDocsEnum) postingsEnum).getNumSubs();
