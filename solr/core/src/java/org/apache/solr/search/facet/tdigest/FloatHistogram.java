...
public class FloatHistogram implements Serializable {
    ...
    public FloatHistogram(double min, double max, double binsPerDecade) {
        ...
        if (binsPerDecade < 5 || binsPerDecade > 10000) {
            throw new IllegalArgumentException(
                    String.format(Locale.ROOT,"Unreasonable number of bins per decade %.2g. Expected value in range [5,10000]",
                            binsPerDecade));
        }
        ...
        if (binCount > 10000) {
            throw new IllegalArgumentException(
                    String.format(Locale.ROOT,"Excessive number of bins %d resulting from min,max,binsPerDecade = %.2g, %.2g, %.2g",
    ...
    public void readObject(java.io.ObjectInputStream in) throws IOException {
        ...
        if (binCount > 10000) {
            throw new IllegalArgumentException(
                    String.format(Locale.ROOT,"Excessive number of bins %d during deserialization = %.2g, %.2g",
                            binCount, min, max));
