...
public class FacetModule extends SearchComponent {
  ...
  public void prepare(ResponseBuilder rb) throws IOException {
    ...
    String jsonFacetStr = params.get("json.facet");
    if (jsonFacetStr == null) return;

    // BP
    if (!rb.req.getCore().getSolrConfig().jsonFacetsEnabled()) {
      throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "JSON Facet API is not enabled.");
    }

    // Do macro expansion for this component only
    ModifiableSolrParams p = new ModifiableSolrParams(params); // make a copy to get get into Map<String,String[]> form
    Map<String,String[]> expanded = MacroExpander.expand(p.getMap());
    ModifiableSolrParams expandedParams = new ModifiableSolrParams(expanded);

    // re-fetch JSON Facet after expansion
    jsonFacetStr = expandedParams.get("json.facet");
    Map<String,Object> jsonFacet =  (Map<String,Object>) ObjectBuilder.fromJSON(jsonFacetStr);
  ...
  public void process(ResponseBuilder rb) throws IOException {
    ...
    if (rb.isDebug()) {
      ...
      fproc.process();
      timer.stop(); long timeElapsed = (long) timer.getTime();
