...
public class TermsComponent extends SearchComponent {
  ...
  protected void checkShardsWhitelist(final ResponseBuilder rb, final List<String> lst) {
    ...
    if (whitelistHostChecker.isWhitelistHostCheckingEnabled() && rb.req.getCore().getCoreDescriptor().getCoreContainer().getZkController() == null && !whitelistHostChecker.hasExplicitWhitelist()) {
      ...
      if (rb.req.getCore().getCoreDescriptor().getCoreContainer().getZkController() != null) {
        cs = rb.req.getCore().getCoreDescriptor().getCoreContainer().getZkController().getClusterState();
      }
